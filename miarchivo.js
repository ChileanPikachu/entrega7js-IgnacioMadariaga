...
function mostrarSwal() {
  Swal.fire({
    icon: "success",
    title: "Buena suerte con su registro",
  })
};
//Termino Sweet Aler
...
let validarEdad = document.getElementById("formularioEdad");
validarEdad.addEventListener("submit", edadvalidada)


function edadvalidada(event) {
  event.preventDefault()
  let verificacion;
  let edad = document.getElementById("inputEdad").value;
  verificacion = edad >= 18 ? true : false;
  verificacion == true ? Swal.fire({
    title: "Eres un adulto, compra con responsabilidad",
  }) : Swal.fire({
    title: "Te sugerimos una tienda infantil",
  });
}
//Termino operador ternario
